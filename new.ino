...
void handleRFIDFast() {
  ...
  if (authorized) {
    ...
    unlockDoor();
    autoLockTime = millis() + 5000; // Auto-lock in 5 seconds
    autoLockActive = true;
    playSuccessBeep(millis());
...
void playSuccessBeep(unsigned long currentTime) {
  digitalWrite(BUZZER_PIN, HIGH);
  buzzerEndTime = currentTime + 200; // 200ms beep
  buzzerActive = true;
}

void playErrorBeep(unsigned long currentTime) {
  digitalWrite(BUZZER_PIN, HIGH);
  buzzerEndTime = currentTime + 500; // 500ms error beep
  buzzerActive = true;
}
...
void showAccessGranted(String employeeName) {
  ...
  int cx = 64;   // Center X
  int cy = 28;   // Center Y
  int r = 10;    // Smaller radius

  display.drawCircle(cx, cy, r, SSD1306_WHITE);
  display.drawLine(cx - 5, cy, cx, cy + 5, SSD1306_WHITE);
  display.drawLine(cx, cy + 5, cx + 7, cy - 3, SSD1306_WHITE);
  ...
  display.setTextSize(1);
  display.setTextColor(SSD1306_WHITE);
  display.setCursor((128 - 13 * 6) / 2, 45); // center approx
  display.println("ACCESS GRANTED");

  // Employee name
  display.setCursor((128 - employeeName.length() * 6) / 2, 55);
